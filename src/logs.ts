...
/** lines 是取到的日志行，脚本没跑成时没有 */
export async function readLogs(cfg: Config, site: string, { tail, since, until }: LogOptions): Promise<{ instance: string; result: RunResult; lines?: string }[]> {
  const n = Number(tail);
  if (!Number.isInteger(n) || n <= 0) throw new Error(`-n must be a positive integer, got "${tail}"`);
  const vars = { NAME: site, TAIL: String(n), SINCE: since ? utc(since, '--since') : '', UNTIL: until ? utc(until, '--until') : '' };
  if (vars.SINCE && vars.UNTIL && vars.SINCE > vars.UNTIL) throw new Error('--since is later than --until');
  const ecs = new Ecs(cfg);
  // 一台服务器查不了（比如关着机）不耽误看别的，它的错误当作云助手的 Error 结果报
  return inParallel(cfg, getSite(cfg, site).instances, async (instance) => {
    try {
