...
- **附带 Agent Skill**：对 Claude Code、Codex 等 AI Agent 说"把 MyApp 发到测试站"，它会先 `--check` 给你看结果，等你确认了再发；说"给每台服务器做个体检"，它能用 `aca run` 查磁盘被什么占满、数据库有没有在备份、多久没装补丁、哪些端口在监听等。
...
`services` 的 key 是 Windows 服务名（`sc query` 列出的那个，不是显示名），字段和站点相同，但没有 `stage` 和 `clb`，另外必填 `dir`：服务器上的安装目录；aca 核对服务跑的程序（可执行文件，用 `dotnet.exe` 启动的是它后面的 `.dll`）确实在这个目录里，对不上就不发。
...
- **系统区域不是英语（美国）的服务器上，输出里 GBK 以外的字符（如 ä、emoji）会变成问号**：云助手客户端在这些服务器上按 GBK 回传输出，中文不受影响。
...
- **只列装在 Windows、Program Files、ProgramData 目录以外的服务**：这三处的是 Windows 自己的和装上的软件（云助手客户端、杀毒、数据库），不是自己发布的程序。用 `dotnet.exe` 启动的服务看它后面的 `.dll` 在哪。
...
- **在服务器本机用 PowerShell 能做的事，都能用 `aca run` 做**：比如查磁盘被什么占满、服务和进程的状态、监听的端口、多久没装补丁，或者重启应用池。实例可以写配置里的别名，也可以写当前地域的任意实例 ID。
- **脚本里有 `$`、引号或换行时用 `--file <文件>`**：写在命令行上的脚本要先经本机 shell 处理，这些字符可能被悄悄改掉；文件要是 UTF-8。
- **`-t` 指定超时秒数**，到点连同脚本启动的进程一起强杀（默认 300）。
...
- **崩溃详情按进程认**：.NET Runtime（异常和调用栈）、Application Error（出错模块、异常代码）、IIS 工作进程的事件，只列这个应用池或服务的进程记下的，ASP.NET Core 进程外托管时也包括它的后端进程；服务按可执行文件的完整路径认，别的目录里同名的程序崩溃不算。用 `dotnet.exe` 启动的服务崩溃时，Application Error 记下的是 dotnet.exe，分不出是哪个程序，aca 看服务控制管理器同一时间有没有报这个服务意外终止。
...
- **服务只比 `.dll`、`.exe` 和 .NET Core 的 `*.runtimeconfig.json`、`*.deps.json`**：服务的程序集和它天天写的日志在同一个目录里。
...
- **预检查还核对服务器满不满足这次发布带来的运行时要求**：下面前两样满足不了时，aca 列出来，哪台服务器都不发；在服务器上装好、调好再发，或者确认没问题加 `-f`。
  ...
  - **.NET Core 的共享框架找不到时只提示**：包带来的 `runtimeconfig.json`（单文件发布的打包在 exe 里，aca 从 exe 里读出来）要的框架，按前滚规则在服务器上找不到。宿主去哪找、认哪个版本还受环境变量（`DOTNET_ROOT`、`DOTNET_ROLL_FORWARD` 等）影响，aca 看不全，所以不拦。
...
aca 在每台服务器上恢复被发布覆盖的文件、删除发布新增的文件和目录、重启站点或服务，然后删掉用过的备份。不给发布 ID 时只撤最近一次发布，再回退一次就撤更早的一次。
...
- **`--from-cloud <证书 ID>` 用数字证书管理服务里的证书**：ID 用 `aca certs cloud` 列，aca 取回 PEM 在本机合成 PFX，密码自己生成，不用 `--password-file`。
