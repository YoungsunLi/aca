...
- **云助手按服务器的系统代码页回传输出**，英文版等非中文 Windows 上站点名、路径和 `-m` 说明里的中文会变成问号，目录路径含中文时 `rollback` 会因此失败。
...
aca 恢复最近一次备份、删除那次新增的文件、重启站点或服务，然后删掉这个备份；再回退一次就退到更早一次发布。回退前在跑的服务要是没起来，说明退到的这一版也起不来：aca 报错，这台服务器上的服务停着，后面的服务器不再回退。先在这台上查清楚：它用掉的那份备份已经删了，再跑一次 `aca rollback` 会跳过它，只把别的服务器退到同一版。站点配了 `clb` 时，回退每台服务器也和发布一样先摘出负载均衡，已经留在外面的服务器先回退；再摘一台就没有服务器接流量时 aca 报错停下，把回退好的服务器用 `aca clb restore` 放回后再回退一次。
