...
try {
  ...
  # 没排除的会整份覆盖服务器上那份，只放行两种：配了 overwriteConfig；ASP.NET Core 发布时生成的 web.config（根路径上配了 aspNetCore），它不放环境值
  foreach ($f in @($configs | Where-Object { -not (Test-AcaExcluded $_.Name $exclude) })) {
    ...
    if (-not $overwrite -and -not ($x -and $x.SelectSingleNode("$acaRoots/*[local-name()='system.webServer']/*[local-name()='aspNetCore']"))) { throw "Package root contains $($f.Name), which would overwrite the environment config on the server; add it to exclude in the aca config, or set overwriteConfig if the package's copy suits every server" }
