...
- **When the pre-check fails on any server** (source code traces, an environment config at the package root, looks like the wrong target, not enough disk space, files older than the copies on the server), aca deploys to none of them and exits. Files older than the copies on the server mean an old build was picked up, or someone edited files on the server; add `-f` if you do want to overwrite them.
